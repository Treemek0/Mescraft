...
MeshData MeshSystem::createChunkData(const Chunk& chunk, const std::array<std::shared_ptr<Chunk>, 6>& neighbors) {
...
MeshData MeshSystem::createChunkData(const Chunk& chunk, const std::unordered_map<uint64_t, std::shared_ptr<Chunk>>& chunkMap) {
    std::vector<Vertex> vertices;
    std::vector<unsigned int> indices;
    unsigned int indexOffset = 0;
            ...
            for(int dx=0; dx<CHUNK_SIZE; dx++){
                ...
                for(int f=0; f<6; f++){
                    ...
                    if(isAir(neighbor.x, neighbor.y, neighbor.z)){
                        ...
                        // Determine which texture to use (side, bottom, or top) based on rotation
                        // 0: Side, 1: Bottom, 2: Top
                        int faceTextureType = 0; // Default to side texture
                        switch (blockData.rotation) {
                            case 0: // Y-axis aligned (default)
                                if (f == 4) faceTextureType = 2; // Top face
                                if (f == 5) faceTextureType = 1; // Bottom face
                                break;
                            case 1: // Z-axis aligned, placed on -Z face
                            case 3: // Z-axis aligned, placed on +Z face
                                if (f == 0 || f == 1) faceTextureType = 2; // Front/Back faces are ends
                                break;
                            case 2: // X-axis aligned, placed on -X face
                            case 4: // X-axis aligned, placed on +X face
                                if (f == 2 || f == 3) faceTextureType = 2; // Left/Right faces are ends
                                break;
                        }

                        // Assign texture atlas x-offset based on type
                        if (faceTextureType == 0) tx = 0;                  // Side
                        else if (faceTextureType == 1) tx = blockTexSize;     // Bottom
                        else if (faceTextureType == 2) tx = blockTexSize * 2; // Top
                        ...
                        if (f == 2) { // -X face on a default block
                            rotateUV(uvs, 90);
                        }
